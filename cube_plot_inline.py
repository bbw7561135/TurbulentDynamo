...
def meetCondition(element):
    ...
    if bool_debug_mode:
        return bool(element.startswith('Turb_hdf5_plt_cnt_') and (int(element.split('_')[-1]) <= 5))
    elif file_max != np.Inf:
        return bool(element.startswith('Turb_hdf5_plt_cnt_') and (int(element.split('_')[-1]) <= file_max))
    else:
        return bool(element.startswith('Turb_hdf5_plt_cnt_'))
...
def plotData(data, temp_iter):
    ...
    plt.colorbar(cbar_plot, shrink=0.8, pad=0.01)
    plt.tight_layout()
    ## set figure range
    ...
    print('\tSaved figure: %i.'%int(temp_iter) + '  \t%0.3f%% complete\n'%(100 * int(temp_iter)/len(file_names)))
    plt.close()
...
filepath_plot = createFilePath([filepath_base, folder_vis, 'plotHDF5_cube']) # where plots will be saved
...
if not(bool_ani_only):
    ...
    for file_name in file_names:
        ...
        ## plot and save data
        print('Plotting data...')
        plotData(temp_data, file_name.split('_')[-1])

##################################################################
## ANIMATING CODE
##################################################################
filepath_input  = (filepath_plot + pre_name +'_'+ type_var + '_%04d_3D.png')
filepath_output = (filepath_plot + '../' + pre_name +'_ani_'+ type_var + '_3D.mp4')
